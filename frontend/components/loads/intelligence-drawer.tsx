...
export function IntelligenceDrawer({ load }: { load: FreightLoad }) {
  ...
  // Broker profile is mocked now; later it can be fetched by broker ID/company ID.
  const profile = mockBrokerProfiles[load.broker];
        ...
        <div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-4">
          ...
          <DrawerBlock icon={ShieldCheck} title="Broker Profile">
            <div className="space-y-2 text-sm">
              <div className="flex justify-between"><span className="text-slate-500">Broker</span><span>{profile.broker}</span></div>
              <div className="flex justify-between"><span className="text-slate-500">Score</span><span className="text-emerald-300">{profile.score}/100</span></div>
              <div className="flex justify-between"><span className="text-slate-500">Days to pay</span><span>{profile.daysToPay}</span></div>
              <div className="flex justify-between"><span className="text-slate-500">Company</span><span>{profile.company}</span></div>
            </div>
          </DrawerBlock>
          ...
          <DrawerBlock icon={BrainCircuit} title="AI Insights">
            <ul className="space-y-2 text-sm text-slate-300">
              <li>Likely bookable at {formatCurrency(Math.round(load.rate * 1.06))} if pickup window is firm.</li>
              <li>Repost probability: <span className="text-amber-300">64%</span> in the next 18 minutes.</li>
              <li>Negotiation angle: ask for detention terms before rate increase.</li>
            </ul>
          </DrawerBlock>

          <DrawerBlock icon={TrendingUp} title="Lane History">
            <div className="flex flex-wrap gap-2">
              {profile.laneHistory.map((lane) => <Badge key={lane} tone="cyan">{lane}</Badge>)}
            </div>
          </DrawerBlock>

          <DrawerBlock icon={History} title="Previous Interactions">
            <Timeline items={profile.previousInteractions} />
          </DrawerBlock>

          <DrawerBlock icon={NotebookPen} title="Notes">
            <Timeline items={profile.notes} />
          </DrawerBlock>
        </div>
      </div>
    </motion.aside>
